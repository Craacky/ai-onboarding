...
1. 
**Upload:** User uploads PDF/DOCX/TXT via React.


2. 
**Storage:** FastAPI saves the file to **MinIO** and marks it 'pending' in **PostgreSQL**.


3. 
**Queue:** A task is sent to **Celery** via **Redis**.


4. 
**Vectorization:** Celery Worker extracts text, creates chunks, generates embeddings via **OpenAI**, and saves them to an isolated **ChromaDB** namespace.


5. 
**Notify:** Status updates to 'ready' and the frontend is notified via **WebSocket**.
...
2. 
**LangChain** retrieves the top-5 relevant chunks from ChromaDB.
...
```text
[cite_start]├── backend/                # FastAPI application [cite: 184]
[cite_start]│   ├── app/api/            # Resource routes [cite: 185]
[cite_start]│   ├── app/models/         # SQLAlchemy models [cite: 187]
[cite_start]│   ├── app/services/       # Business logic (LangChain, Stripe) [cite: 189]
[cite_start]│   └── app/tasks/          # Celery tasks [cite: 190]
[cite_start]├── frontend/               # React application [cite: 192]
[cite_start]├── nginx/                  # Nginx configuration [cite: 193]
[cite_start]├── .github/workflows/      # CI/CD pipelines [cite: 194]
[cite_start]└── docker-compose.yml      # Local development setup [cite: 195]
